...
class TransformerDiscriminator(nn.Module):
    def __init__(self, params):
        ...
        self.embedding = nn.Sequential(
            nn.Linear(in_features + 1, embedding_dim), nn.LeakyReLU()
        )
    ...
    def forward(self, x, y):
        ...
        y_expand = y.repeat(1, T)
        y_expand = y_expand.unsqueeze(-1)
        # ipdb.set_trace()
        x = torch.cat([x, y_expand], dim=2)
        x = self.embedding(x)

        batch_size, tweet_length, embedding_dim = x.shape

        conditional_pos = self.cond_embedding(y).repeat(1, tweet_length, 1)
